"""
Finnhub Earnings Surprises Service
Service to fetch historical quarterly earnings surprises using Finnhub API
Uses /stock/earnings endpoint
"""
import requests
import asyncio
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                if response.status_code == 401:
                    ...
                    print("   2. Endpoint /stock/earnings may require specific subscription")
...
async def get_earnings_history(symbol: str, limit: Optional[int] = None) -> Optional[Dict]:
    """
    Fetch historical quarterly earnings surprises from Finnhub API for a given symbol.
    Uses /stock/earnings endpoint.

    Free tier: last 4 quarters. Premium: full history (limit optional).
    """
    ...
    print(f"📊 Fetching Earnings Surprises from Finnhub for {symbol}...")
    earnings_data = await loop.run_in_executor(None, fetch_earnings)

    if not earnings_data or isinstance(earnings_data, Exception):
        print(f"⚠️ Failed to fetch Earnings Surprises for {symbol}")
        return None

    if not isinstance(earnings_data, list):
        print(f"⚠️ Unexpected earnings response format: {type(earnings_data)}")
        return None
