"""
Finnhub Basic Financials Service
Service to fetch basic financials data using Finnhub API
Uses /stock/metric endpoint
"""
import requests
import asyncio
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                if response.status_code == 401:
                    ...
                    print("   3. Endpoint /stock/metric may require specific subscription")
...
async def get_basic_financials(symbol: str, metric: str = "all") -> Optional[Dict]:
    """
    Fetch Basic Financials data from Finnhub API for a given symbol.
    Uses /stock/metric endpoint.
    """
    ...
    def fetch_basic_financials():
        ...
        return safe_get_sync(url, params={"symbol": symbol, "metric": metric, "token": FINNHUB_API_KEY})

    print(f"📊 Fetching Basic Financials from Finnhub for {symbol} (metric={metric})...")
    basic_data = await loop.run_in_executor(None, fetch_basic_financials)

    if not basic_data or isinstance(basic_data, Exception):
        print(f"⚠️ Failed to fetch Basic Financials for {symbol}")
        return None

    # Add simple metadata wrapper (optional)
    wrapped = {
        "basic_financials": basic_data,
        "_metadata": {
            "symbol": symbol,
            "metric": metric,
