...
Service to fetch market news data using Finnhub API
Uses /news endpoint
"""
import requests
import asyncio
...
# Valid categories for news endpoint
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                if response.status_code == 401:
                    ...
                    print("💡 Status 404: Endpoint not found or invalid category")
...
async def get_market_news(category: str, min_id: int = 0) -> Optional[List[Dict]]:
    """
    Fetch Market News data from Finnhub API.
    Uses /news endpoint.
    
    Args:
        category: News category (general, forex, crypto, merger)
        min_id: Minimum news ID to fetch (default: 0)
    ...
    print(f"📰 Fetching Market News from Finnhub (category={category}, min_id={min_id})...")
    news_data = await loop.run_in_executor(None, fetch_news)

    if not news_data or isinstance(news_data, Exception):
        print(f"⚠️ Failed to fetch Market News for category {category}")
        return None
